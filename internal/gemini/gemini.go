// Package gemini wraps Google's Generative AI API to parse free-text
// in-game trade messages into a structured list of items.
package gemini
...
// TradeItem is one parsed item from a free-text trade message.
...
// TradeResult is the structured output from ParseTradeMessage.
...
- replace the word: "peco peco" to "PecoPeco", "cavalo marinho" to "Cavalo-Marinho", "louva a deus" to "Louva-a-Deus", "Besouro Ladrão" to "Besouro-Ladrão", "Bigode de Toupeira" to "Bigodes de Toupeira", "biblia" to "Bible", "oca" to "Old Card Album"
...
// Client holds the API key for Gemini. It is safe for concurrent use; each
// call to ParseTradeMessage creates a new short-lived genai client.
type Client struct {
	apiKey string
}

// New returns a Client. apiKey must be non-empty.
func New(apiKey string) *Client { return &Client{apiKey: apiKey} }

// ParseTradeMessage sends `message` to Gemini and returns the parsed
// trade items.
func (c *Client) ParseTradeMessage(message string) (*TradeResult, error) {
	if c == nil || c.apiKey == "" {
